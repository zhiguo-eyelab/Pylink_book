...
class EyeLinkCoreGraphicsPsychoPy(pylink.EyeLinkCustomDisplay):
    def __init__(self, tracker, win):
        ...
        self.calibTar = visual.Circle(self.display, radius=15, lineWidth=8, lineColor='black', fillColor='white', units='pix')
    ...
    def draw_cal_target(self, x, y):
        ...
        if self.calTarget is 'default':
            ...
            if self.calTarget is 'rotatingCheckerboard':
                self.calibTar = visual.RadialStim(self.display, tex = 'sqrXsqr', mask='circle', radialCycles=2,angularCycles=6, size=1.0/20*self.w)
            if self.calTarget is 'movie':
                self.calibTar = visual.MovieStim3(self.display, self.movieTargetFile, loop=True)
