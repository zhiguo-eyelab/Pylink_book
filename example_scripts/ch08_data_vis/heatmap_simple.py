...
os.system('edf2asc -e -y -res %s' %(edf_dir + 'zw.edf')) # convert the EDF file
...
for line in asc:
    ...
    if re.search('SYNCTIME', line): # message marking image onset
        ...
        alpha = 0.5  # transparancy for the heatmap 
        w, h = np.meshgrid(np.linspace(0,scn_w,scn_w), np.linspace(0,scn_h,scn_h))
        heatmap = 0.0*np.exp(-1.0*(w-0)**2/(2*sigma_x**2)-1.0*(h-0)**2/(2*sigma_y**2))
    ...
    if trial_start:
        if re.search('^EFIX', line): # add fixation summary data to efix
            ...
            heatmap += duration*np.exp(-1.0*(w-gaze_x)**2/(2*sigma_x**2)-1.0*(h-gaze_y)**2/(2*sigma_y**2))

        if re.search('DRAW_LIST', line): # get background image from the .VCL file
            ...
            for draw_commands in vcl:
                if 'IMGLOAD' in draw_commands:
                    ...
                    bg_image = draw_commands.split()[3] # get the path to the .png image
    ...
    if re.search('blank_screen', line): # message marking image offset
        ...
        # apply a colormap (from the colormap library in MatplotLib) to the heatmap
        heatmap = heatmap/np.max(heatmap)
        heatmap = Image.fromarray(np.uint8(cm.seismic(heatmap)*255)).convert('RGBA')
        heatmap = Image.blend(background_pic, heatmap, alpha) # blending
        heatmap.save('heatmap_trial_%d.png'%trial_number, 'PNG') # save the heatmap
        trial_start = False
